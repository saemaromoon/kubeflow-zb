...
if [ $key_length -eq '21' -a $secret_length -eq '41' ] ; then
    ...
    docker build -t test -f Dockerfile .. --build-arg DISABLE_CACHE=$(date +%s) --build-arg AWS_ACCESS_KEY_ID=$AWS_ACCESS_KEY_ID --build-arg AWS_SECRET_ACCESS_KEY=$AWS_SECRET_ACCESS_KEY
    exit 0
fi
